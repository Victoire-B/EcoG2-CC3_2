...
pp + geom_bar(aes(color=Phylum, fill=Phylum), stat="identity", position="stack")
```
...
ps.top20 <- transform_sample_counts(ps, function(OTU) OTU/sum(OTU))
ps.top20 <- prune_taxa(top20, ps.top20)
pg <- plot_bar(ps.top20, x="Rendement", fill="Genus", title="Diversité des genre les plus abondants") +  facet_wrap(~Rendement, scales = "free")
pg + geom_bar(aes(color=Genus, fill=Genus), stat="identity", position="stack")
```
